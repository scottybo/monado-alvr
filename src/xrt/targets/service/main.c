...
// ALVR compositor target registration.
// Declared here so comp_main doesn't need to link against drv_alvr.
extern const struct comp_target_factory comp_target_factory_alvr;
void comp_compositor_register_alvr_factory(const struct comp_target_factory *ctf);

static void __attribute__((constructor))
alvr_register(void)
{
	comp_compositor_register_alvr_factory(&comp_target_factory_alvr);
}
